      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 border-b border-border/40 flex items-center justify-between px-6 bg-card/10 backdrop-blur-md z-20 shrink-0">
          ...
          <div className="flex items-center space-x-4 relative">
            ...
            {searchResults && (
              <div className="absolute top-12 right-12 w-80 bg-zinc-950 border border-border shadow-2xl rounded-2xl p-4 z-40 space-y-3">
                ...
                {searchResults.companies.length === 0 && searchResults.passports.length === 0 && searchResults.documents.length === 0 && (
                  <p className="text-[10px] text-muted-foreground text-center py-2">No matching registry found.</p>
                )}
              </div>
            )}

            {/* Notification alert bells */}
            <div className="relative p-1.5 hover:bg-neutral-800/40 rounded-xl transition-colors cursor-pointer" title={`${unreadAlertsCount} pending verifications`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 relative radial-glow">
          <div className="max-w-6xl mx-auto space-y-6 pb-12">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              {/* Side Inspector (If item selected) */}
